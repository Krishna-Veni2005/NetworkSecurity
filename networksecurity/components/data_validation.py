...
import os,sys
import pandas as pd
from scipy.stats import ks_2samp   # It used to chec wheather we have any data drift or not
...
class DataValidation:
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            # Reading the train and test file from data ingestion artifact
            train_dataframe=DataValidation.read_data(train_file_path)
            test_dataframe=DataValidation.read_data(test_file_path)

            ## validate numner of columns
            ...
            num_cols=train_dataframe.select_dtypes(include='number').columns
            if len(num_cols)==0:
                error_message=f"Train dataframe does not contain any numerical columns.\n"

            num_cols=test_dataframe.select_dtypes(include='number').columns
            if len(num_cols)==0:
